...
class EqualLowDetector:
    """
    Detects confirmed equal-low liquidity from swing lows.

    The detector compares every eligible pair of confirmed swing
    lows rather than only adjacent lows.

    Responsibilities:
    - identify swing-low pairs within the configured tolerance;
    ...
    def __post_init__(self) -> None:
        if self.tolerance < 0.0:
            raise ValueError(
                "EqualLowDetector tolerance cannot be negative."
            )

        if self.min_separation < 1:
            raise ValueError(
                "EqualLowDetector min_separation must be at least 1."
            )

        if self.ideal_separation < self.min_separation:
            raise ValueError(
                "EqualLowDetector ideal_separation cannot be less "
                "than min_separation."
            )
    ...
    def detect(
        ...
        """
        Detect equal-low liquidity from confirmed swings.

        All confirmed LOW swings are compared with later LOW swings.
        ...
        for first_position in range(len(low_swings) - 1):
            first = low_swings[first_position]

            for second_position in range(
                first_position + 1,
                len(low_swings),
            ):
                second = low_swings[second_position]

                if not self._is_equal_low_pair(
                    first=first,
                    second=second,
                ):
                    continue
    ...
    def _build_finding(
        ...
        *,
        ...
        return LiquidityFinding(
            ...
            reason=(
                "Two confirmed swing lows formed within "
                f"{price_difference:.4f} of one another across "
                f"{separation} bars, creating resting sell-side "
                "liquidity."
            ),
            evidence=(
                (
                    "First confirmed swing low occurred at "
                    f"{first.price:.4f} on index {first.index} "
                    f"with strength {first.strength}."
                ),
                (
                    "Second confirmed swing low occurred at "
                ...
                (
                    "The swing lows were separated by "
                    f"{separation} bars."
                ),
            ),
